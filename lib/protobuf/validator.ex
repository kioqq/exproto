defmodule Protobuf.Validator do
  ...
  def valid?(%{__struct__: mod} = struct) do
    ...
    valid = if MessageProps.has_oneof?(msg_props), do: oneof_valid?(struct, msg_props), else: true
      ...
      Enum.reduce_while(msg_props.field_props, true, fn {_, props}, _ ->
        if field_valid?(ctx, props, Map.get(struct, props.name_atom)) do
          ...
          {:halt,
           {:invalid,
            """
            #{inspect(mod)}##{props.name_atom} is invalid!
  ...
  defp type_valid?(:enum, val) when is_atom(val), do: true
    ...
    Enum.reduce_while(msg_props.oneof, true, fn {oneof_name, index}, _ ->
      case Map.get(struct, oneof_name) do
        {field, val} ->
          ...
          check_oneof_field(field, oneof_name, field_props, index, ctx, val)
  ...
  defp check_oneof_field(field, oneof_name, field_props, index, ctx, val) do
    cond do
